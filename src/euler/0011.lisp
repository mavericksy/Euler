 ...
 (print
  (max
   ...
   (loop :for row :from 0 :below 20
         :maximizing (loop :for col :from 0 :below 16
                           :maximizing (* (aref grid row (+ 0 col))
                                          (aref grid row (+ 1 col))
                                          (aref grid row (+ 2 col))
                                          (aref grid row (+ 3 col)))))
   ;; vertical
   (loop :for row :from 0 :below 16
         :maximizing (loop :for col :from 0 :below 20
                           :maximizing (* (aref grid (+ 0 row) col)
                                          (aref grid (+ 1 row) col)
                                          (aref grid (+ 2 row) col)
                                          (aref grid (+ 3 row) col))))
   ;; diagonal l-r
   (loop :for row :from 0 :below 16
         :maximizing (loop :for col :from 0 :below 16
                           :maximizing (* (aref grid (+ 0 row) (+ 0 col))
                                          (aref grid (+ 1 row) (+ 1 col))
                                          (aref grid (+ 2 row) (+ 2 col))
                                          (aref grid (+ 3 row) (+ 3 col)))))
   ;; diagonal r-l
   (loop :for row :from 3 :below 20
         :maximizing (loop :for col :from 0 :below 16
                           :maximizing (* (aref grid (- row 0) (+ 0 col))
